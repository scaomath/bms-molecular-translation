...
def run_train():
    ...
    initial_checkpoint = \
        out_dir + '/checkpoint/00052000_model.pth'#
            ...
            if is_mixed_precision:
                with amp.autocast():
                    ...
                    if 0: #use wrong prediction as input
                        ...
                        a = (torch.rand(token[:, 1:].shape)>0.8).float().cuda() #<todo> sample according to probability

                        fake_token = token.clone()
                        fake_token[:, 1:] = a*fake_token[:, 1:] + (1-a)*predict[:,:-1]
                        logit1 = data_parallel(net, (image, fake_token, length))
